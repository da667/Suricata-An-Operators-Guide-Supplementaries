...
print_status "modifying permissions to allow the suricata user and group access to rules, config files, touching local.rules into existence, etc."

dir_check /usr/local/var/run/suricata

touch /usr/local/var/lib/suricata/rules/local.rules &>> $logfile
error_check 'touch of /usr/local/var/lib/suricata/rules/local.rules'

print_status "enabling local.rules.."
sed '-\ suricata\.rules/a\ \ -\ local\.rules' /usr/local/etc/suricata/suricata.yaml &>> $logfile
...
chown -R suricata:suricata /usr/local/etc/suricata &>> $logfile
error_check 'suricata user ownership of /usr/local/etc/suricata'
chown -R suricata:suricata /usr/local/var/run/suricata &>> $logfile
...
print_notification "Wanna update your rules? run suricata-update"
